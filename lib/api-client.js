...
class ApiClient {
  ...
  getBlogPostsFromStorage() {
    try {
      const storedData = localStorage.getItem("blog_posts")
  ...
  saveBlogPostToStorage(postData) {
    try {
      const currentData = this.getBlogPostsFromStorage()
      const timestamp = Date.now()
      const newPost = {
        ...(postData instanceof FormData ? Object.fromEntries(postData.entries()) : postData),
      ...
      localStorage.setItem("blog_posts", JSON.stringify({ posts: currentData, timestamp }))
  ...
  updateBlogPostInStorage(id, postData) {
    try {
      ...
      localStorage.setItem("blog_posts", JSON.stringify({ posts: currentData, timestamp: Date.now() }))

      return { post: currentData[index] }
    } catch (error) {
  ...
  deleteBlogPostFromStorage(id) {
    try {
      const currentData = this.getBlogPostsFromStorage()
      const filteredPosts = currentData.filter((p) => p.id !== id)

      localStorage.setItem("blog_posts", JSON.stringify({ posts: filteredPosts, timestamp: Date.now() }))
